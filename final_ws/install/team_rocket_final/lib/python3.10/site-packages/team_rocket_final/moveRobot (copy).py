...
class MoveRobot(Node):
    def __init__(self):
        ...
        # ========== DECLARE PARAMETERS FOR DEBUGGING ==========
        self.declare_parameter('debug_mode', False)  # Set to True for teleop testing
        self.declare_parameter('detection_frequency', 2.0)  # Hz for CNN detection
        ...
        if self.debug_mode:
            ...
            self.get_logger().info('🔧 DEBUG MODE: Autonomous navigation DISABLED')
            self.get_logger().info('   Use teleop to drive and test CNN detection!')
            self.get_logger().info('='*60)
        else:
            self.get_logger().info('='*60)
            self.get_logger().info('🤖 AUTONOMOUS MODE: Full navigation enabled')
            self.get_logger().info('='*60)
        ...
        # Class names with icons for display
        self.class_display = {
            'empty': ('⚪', 'Empty'),
            'left': ('⬅️ ', 'Left'),
            'right': ('➡️ ', 'Right'),
        ...
        # ========== Timers ==========
        # Navigation timer - only active in autonomous mode
        timer_period = 0.1
        if self.debug_mode:
            self.get_logger().info('⏸️  Navigation timer disabled (debug mode)')
            # Don't create navigation timer in debug mode
            self.timer = None
        else:
            self.timer = self.create_timer(timer_period, self.navigation)
            self.get_logger().info('✓ Navigation timer active')
        
        # Detection timer - always active for testing
        detection_period = 1.0 / self.detection_freq
        self.detection_timer = self.create_timer(detection_period, self.periodic_detection)
        self.get_logger().info(f'✓ Detection timer active ({self.detection_freq} Hz)')
    ...
    def periodic_detection(self):
        """
        Periodic detection callback for testing
        Runs continuously at configured frequency
        """
        if self.latest_image is None:
            return
        ...
        # Keep history
        self.detection_history.append((class_name, conf_value))
        if len(self.detection_history) > 10:
            self.detection_history.pop(0)
        
        # Get display info
        icon, display_name = self.class_display.get(class_name, ('❓', class_name))
        
        # Color codes for terminal
        color_codes = {
            'empty': '\033[90m',      # Gray
            'left': '\033[92m',       # Green
            'right': '\033[93m',      # Yellow
            'do_not_enter': '\033[91m',  # Red
            'stop': '\033[91m',       # Red
            'goal': '\033[95m'        # Magenta
        }
        color = color_codes.get(class_name, '\033[0m')
        reset = '\033[0m'
        
        # Log detection with formatting
        self.get_logger().info(
            f'{icon} {color}[{display_name:^15}]{reset} '
            f'Confidence: {conf_value*100:5.1f}% '
            f'(#{self.detection_count})'
        )
        
        # Publish detection result
        detection_msg = String()
        detection_msg.data = f'{class_name},{conf_value:.3f}'
    ...
    def navigation(self):
        # Safety check - don't navigate in debug mode
        if self.debug_mode:
            return
